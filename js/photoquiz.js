$( document ).ready(function() {
	...
	function gameBegin( gameData ) {
		...
		G.numMoves = 0;
		G.slides = gameData[0].slides;
		G.slideDuration = gameData[0].slideduration;
		G.headline = gameData[0].headline;
		G.okToGuess = true;
		G.wrongAnswers = 0;
		G.totalMatches = 0;
		G.currentSlide = 0;
		...
		if (gameData[0].randomizeOrder == true) {
			//clone and shuffle
		...
		function loadInstructions() {
			...
			$(".intro").append('<p><strong>How it works</strong>The faster you answer, the more points you get. But be careful - wrong guesses deduct points.</p>');
			$("#container").append('</div>');
			$( window ).trigger( gameEventName, [ 'loaded' ] );
			$(".start").click(function(event) {
				...
				$( window ).trigger( gameEventName, [ 'started' ] );
		...
		function showEndScreen() {
			...
			$( window ).trigger( gameEventName, [ 'completed' ] );
		}
		...
		function showFeedback(answer, scoreForDisplay) {
			...
			if (answer == "correct") {
				...
				content += '<span class="incorrect-holder"><span class="incorrect">Incorrect!</span><span class="points">-'+G.pointsPerSlide+' Points</span></span>';
			}
	...
	$( window ).on( gameEventName, function( event, message ) {
		...
		if ( typeof mobileAnalyticsClient == "object" ) {
			...
			mobileAnalyticsClient.recordEvent( 'pixels activity', {
				'user_action': message,
				}, {
				});
